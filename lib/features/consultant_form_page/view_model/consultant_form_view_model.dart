...
import 'package:flutter_app_demo/features/consultant_form_page/services/api_service.dart';
...
class FormViewModel extends ChangeNotifier {
  ...
  Future<void> submitForm() async {
    ...
    try {
      var response = await ApiCallMethods.post(
        url: "https://stgtmfapp.dhwaniris.in/api/v2/method/consultant-signup",
        data: apiBody,
      );

      var mainResponse = ApiCallMethods.checkResponse(response: response);

      showToast(mainResponse.message ?? "Submitted successfully!");

    } on ApiException catch (e) {
      showToast(e.toString());
    } catch (e) {
      printText("consultant view model error :$e");
